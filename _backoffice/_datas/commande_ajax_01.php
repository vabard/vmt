...
if ($_POST['id'] == -2) {
    ...
    while($extract = $rqselect -> fetch(PDO::FETCH_ASSOC)){
        ...
        // $tab['liste'] .= '<div class="divchamp" style="width:10%;"> ' . $extract['id_produit'] .'</div>';
            ...
            $laproduit = '<div class="divchamp" style="width:15%;"> ';
            $laproduit .= $produit['id_salle'] . ' ' . $produit['prix'] . '</div>';
        $tab['liste'] .= $laproduit;
        $tab['liste'] .= '<div class="divchamp" style="width:40%;"> ' . $extract['id_membre'] .'</div>';
        $tab['liste'] .= '<div class="divchamp" style="width:5%;"> ' . $extract['id_produit'] .'</div>';
...
if ($_POST['id'] != -1) {
    ...
    if($rqselect -> rowCount() > 0){
        ...
        $tab['fiche'] .= '<input type="text" class="" style="width:25%;" id="id_commande" name="id_commande" value="' . $enr['id_commande'] .'" >';
